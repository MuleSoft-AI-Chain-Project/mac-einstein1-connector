...
public class AgentforceBotOperations {
   ...
   * 
   * @OutputJsonType(schema = "api/response/StartAgentConversationResponse.json") public Result<InputStream,
   * InvokeAgentResponseAttributes> startAgentConversation(@Connection AgentforceConnection connection,
   * 
   * @Placement(order = 1) @ParameterGroup( name = "Agent") @MetadataKeyId BotAgentParameterGroup parameterGroup) {
   * 
   * log.info("Executing start agent conversation operation.");
   * 
   * try { AgentConversationResponseDTO responseDTO = connection.getBotRequestHelper().startSession(parameterGroup.getAgent());
   * JSONObject jsonObject = new JSONObject(); jsonObject.put("sessionId", responseDTO.getSessionId());
   * 
   * return Result.<InputStream, InvokeAgentResponseAttributes>builder() .output(toInputStream(jsonObject.toString(),
   * StandardCharsets.UTF_8)) .attributes(responseDTO.getResponseAttributes())
   * .attributesMediaType(org.mule.runtime.api.metadata.MediaType.APPLICATION_JAVA)
   * .mediaType(org.mule.runtime.api.metadata.MediaType.APPLICATION_JSON) .build(); } catch (Exception e) {
   * 
   ...
   * @Throws(BotErrorTypeProvider.class)
   * 
   * @OutputResolver(output = AgentConversationResponseMetadataResolver.class) public Result<InputStream,
   * InvokeAgentResponseAttributes> continueConversation(@Connection AgentforceConnection connection,
   * 
   * @Content(primary = true) InputStream message,
   * 
   * @Content String sessionId,
   * 
   * @Summary("Increase this number for each subsequent message in a session") @DisplayName("Message Sequence Number") int
   * messageSequenceNumber) { log.info("Executing continue agent conversation operation.");
   * 
   * try { AgentConversationResponseDTO responseDTO = connection.getBotRequestHelper().continueSession(message, sessionId,
   * messageSequenceNumber); return Result.<InputStream, InvokeAgentResponseAttributes>builder()
   * .output(toInputStream(responseDTO.getText(), StandardCharsets.UTF_8)) .attributes(responseDTO.getResponseAttributes())
   * .attributesMediaType(org.mule.runtime.api.metadata.MediaType.APPLICATION_JAVA)
   * .mediaType(org.mule.runtime.api.metadata.MediaType.TEXT) .build(); } catch (Exception e) {
   * 
   * throw new ModuleException("Error in continue agent conversation for session id: " + sessionId, AGENT_OPERATIONS_FAILURE, e);
   * } }
   * 
   */
  /*
   ...
   * 
   * @Summary("Increase this number for each subsequent message in a session") @DisplayName("Message Sequence Number") int
   * messageSequenceNumber, CompletionCallback<InputStream, InvokeAgentResponseAttributes> callback) {
